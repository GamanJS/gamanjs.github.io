---
title: Middlewares
description: Middleware in GamanJS runs before the route handler for checks, context modification, or request blocking.
---
...
If middleware needs to stop the request (e.g., auth failure), **don't call `next()`**, return a response using `ctx.send()`:
